...
async function keepaliveNotes({ fingerprint, baseUrl = TECHNOCORE_URL } = {}) {
  const fp = String(fingerprint || "").trim().toLowerCase();
  if (!/^[a-f0-9]{16}$/.test(fp)) {
    throw new Error("fingerprint must be 16 hex chars");
  }
  const base = normalizeBaseUrl(baseUrl);
  const profilePath = `/kv/did-${fp.slice(0, 2)}/${fp.slice(2)}`;
  const contribPath = `/kv/contrib/${fp}`;

  async function refresh(path) {
    const read = await fetchText(`${base}${path}`);
    if (!read.ok) {
      throw new Error(`read failed ${path}: ${read.status} ${read.body.slice(0, 180)}`);
    }
    const line =
      read.body
        .split("\n")
        .map((x) => x.trim())
        .find((x) => x.startsWith("technocore-")) || "";
    if (!line) {
      throw new Error(`no technocore-* payload in ${path}`);
    }
    const setUrl = `${base}${path}/set/${encodeURIComponent(line)}`;
    const write = await fetchText(setUrl);
    return {
      path,
      readStatus: read.status,
      writeStatus: write.status,
      writeBody: write.body.slice(0, 240),
      bytes: Buffer.byteLength(line, "utf8"),
      preview: line.slice(0, 160),
    };
  }

  const profile = await refresh(profilePath);
  const contrib = await refresh(contribPath);
  return {
    fingerprint: fp,
    didNote: `${base}${profilePath}`,
    contribNote: `${base}${contribPath}`,
    profile,
    contrib,
